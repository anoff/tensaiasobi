...
type Screen = 'menu' | 'math' | 'odd' | 'doodle' | 'memory' | 'maze' | 'trace' | 'letterTrace' | 'anlaut' | 'emojiMatch' | 'town' | 'coupons' | 'settings' | 'shiritori' | 'puzzle' | 'dispatch' | 'physics' | 'towerSort' | 'shadowFlashlight';
...
interface LauncherDef {
  ...
  testid: string;
  emoji: string;
  label: 'math' | 'odd' | 'doodle' | 'match' | 'maze' | 'trace' | 'letterTrace' | 'dobble' | 'anlaut' | 'shiritori' | 'puzzle' | 'dispatch' | 'physics' | 'towerSort' | 'shadowFlashlight';
}
...
function AppContent() {
  ...
  const { coupons, toggleCoupon, awardCoupon, resetCoupons } = useCoupons();
  const [pendingCouponAwardId, setPendingCouponAwardId] = useState<string | null>(null);
    ...
    switch (currentScreen) {
      ...
      case 'coupons':
        return (
          <CouponShop
            coupons={coupons}
            onAwardCoupon={(id) => setPendingCouponAwardId(id)}
            playPop={playPop}
          />
        );
      case 'settings':
  ...
  return (
    <div className="w-screen h-[100dvh] flex flex-col bg-sky-50 text-slate-800 relative pt-safe pb-safe">
      ...
      {/* Parent Gate for Direct Coupon Award */}
      {pendingCouponAwardId && (
        <ParentGate
          onSuccess={() => {
            const success = awardCoupon(pendingCouponAwardId);
            if (success) {
              playSuccess();
            } else {
              playError();
            }
            setPendingCouponAwardId(null);
          }}
          onClose={() => setPendingCouponAwardId(null)}
        />
      )}
